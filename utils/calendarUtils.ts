...
// Internal helper to format dates for calendar APIs (ICS and Google Link)
// Returns strings in format YYYYMMDDTHHmmSSZ
...
export const generateGoogleCalendarUrl = (race: F1Race): string => {
  const { start, end } = formatDateToICS(race.date, race.time);
  
  const title = encodeURIComponent(`F1 ${race.grandPrixName}`);
  const details = encodeURIComponent(`Formula 1 Round ${race.round}\nCircuit: ${race.circuitName}\n\n${race.description}\n\n(Generated by F1 Sync 26)`);
  const location = encodeURIComponent(`${race.circuitName}, ${race.city}, ${race.country}`);
  
  // Google Calendar Render URL
  return `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${title}&dates=${start}/${end}&details=${details}&location=${location}&sprop=website:f1sync26.app`;
};
